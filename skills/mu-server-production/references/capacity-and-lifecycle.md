...
Use `request.remoteAddress()` for a direct peer. `request.clientIP()` accepts forwarding metadata and is safe as a security bucket only when the immediate proxy is trusted and removes spoofed input. Rate-limit selectors run while Mu creates the exchange, before the handler chain, so an earlier `MuHandler` cannot establish that trust for a selector. A selector that uses `clientIP()` must itself validate `request.connection().remoteAddress()` first, or use one fixed quarantine bucket for every unexpected peer; network policy must still prevent clients from reaching an ingress-only listener directly. At a public edge, prefer an ingress/WAF for globally coordinated abuse controls. Test bucket cardinality and expiry so attacker-controlled bucket names do not create unbounded state.
