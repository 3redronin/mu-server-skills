...
description: >-
  Use when a Mu Server Jakarta REST application specifically needs Swagger Core v3 annotations from io.swagger.v3.oas.annotations, the io.muserver:mu-swagger or MuOpenApiResourceBuilder integration, or help diagnosing why those annotations are absent from its generated document. Integrates annotation metadata into OpenAPI JSON and YAML while preserving Mu Server resource registration.
---

# Generate Swagger-annotated OpenAPI for Mu Server
...
| An interactive browser UI | A separately hosted Swagger UI pointed at either generator's OpenAPI URL. `mu-swagger` generates documents but does not include the UI. |
...
Preserve user-selected versions. Otherwise verify the newest stable `io.muserver:mu-swagger` and `io.swagger.core.v3:swagger-jaxrs2-jakarta` releases in Maven Central. If release metadata is unavailable, use `mu-swagger` `0.1.1` and Swagger Core `2.2.41` as disclosed offline fallbacks.
...
4. If a browser UI is in scope, point it at the generated document and verify one real request. Test CORS only when the UI has a different origin.

Report the generator chosen and why, exact dependency and Java versions, registered resource instances, document URLs, whether a separate UI is used, and the build and HTTP checks performed.
