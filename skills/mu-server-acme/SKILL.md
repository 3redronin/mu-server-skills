...
`mu-acme` supports HTTP-01 only. Add each exact DNS name with a separate `withDomain(...)`; the manager requests one SAN certificate and every name must validate. HTTP-01 cannot issue wildcard certificates, so a wildcard or DNS-01 requirement needs a different ACME client. A wildcard such as `*.example.com` also does not cover the apex `example.com` or deeper names such as `a.b.example.com`; request the identifiers the deployment actually serves and minimize distribution of the broader wildcard private key.

Before a real order, confirm that every configured A and AAAA record reaches this deployment, public inbound TCP 80 reaches the Mu HTTP handler, and the manager can make outbound HTTPS connections to the ACME service. Public 443 is required for the application's HTTPS service and for any HTTP-01 redirect to HTTPS, but not when the challenge is answered directly on port 80. A private alternate HTTP listener is fine only when the public edge forwards port 80 without intercepting the challenge path.
...
4. Call `start` exactly once; the implementation does not guard repeated calls, and each call schedules another daily task. On application shutdown, call `certManager.stop()` before stopping the server; it waits up to 30 seconds for an in-flight renewal, so the surrounding shutdown must still account for work that outlives that wait. Build a new manager rather than trying to restart a stopped one.

Mu Server's `MuHandler` contract and current dispatcher execute handlers strictly in registration order. A stale sentence in `MuServerBuilder.addHandler` Javadocs says async handlers are reordered first; `handleAsync()` changes completion ownership, not handler position. Any earlier synchronous or asynchronous handler can consume the request, so put the ACME handler first and also verify that the ingress, WAF, and any server-level limiter allow the challenge. Let's Encrypt currently follows a limited set of HTTP-01 redirects on ports 80 and 443 without validating the bootstrap HTTPS certificate, but direct handler-first service is the portable arrangement and avoids auth, rewrite, cache, and CA-policy dependencies.
