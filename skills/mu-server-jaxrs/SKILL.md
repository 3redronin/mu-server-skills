...
description: >-
  Create, change, or troubleshoot application code that hosts Jakarta REST/JAX-RS APIs with mu-server's built-in RestHandlerBuilder. Use when a project combines io.muserver with jakarta.ws.rs resources and needs singleton construction, explicit provider registration, filters, readers or writers, exception mapping, CORS, OpenAPI, or HTTP verification.
---
...
Mu Server's Jakarta REST implementation uses application-created singleton objects. Initialize the `MuServerBuilder` first so Mu's Jakarta REST runtime delegate is installed. Then construct each resource, filter, interceptor, mapper, reader, writer, and converter once, wire its dependencies through constructors, and pass the instance to `RestHandlerBuilder`.

Mu Server does not inject resource constructors or fields, create a resource per request, scan the classpath, or discover a class merely because it has `@Provider`. An existing dependency-injection container can construct an object, but the application still passes that singleton instance to the REST builder. Make application-owned singletons thread-safe because concurrent requests can call the same instance.
...
For a new plain-text API, use this shape and adapt names, paths, messages, and port to the request:
...
Use `jakarta.ws.rs` imports with mu-server 2.x. Add a provider dependency only when explicitly requested behavior needs one, such as converting arbitrary objects to and from JSON.
